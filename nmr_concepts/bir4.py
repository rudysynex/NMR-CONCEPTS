...
def bir4_rf(
    ...
    """
    Generate a B1-insensitive rotation (BIR-4) pulses
    ...
    if func == "sech/tanh":
        ...
        phs_mod = np.concatenate(
            [phs_mod, np.flip(phs_mod) + 180 + nut_angle / 2.0]
        )
        ...
        # Define the shape (first segment)
        rf_form = np.tanh(rf_mod * (1.0 - t_vec))
